...
const BookingTransport = () => {
    ...
    const [selectedBookingId, setSelectedBookingId] = useState(null); // Nouveau state pour stocker l'ID de la réservation sélectionnée
    ...
    const fetchTransportBookings = async () => {
        try {
            const response = await axios.get('/transport/bookings');
    ...
    const handleEdit = (id) => {
        navigate(`/account/bookings-transport/${id}/edit`);
    };
    ...
    return (
        <div className="mt-5 min-h-screen bg-gradient-to-r from-blue-50 to-purple-100">
            ...
            <div className="container mx-auto px-4 py-8">
                <h1 className="text-3xl font-bold text-center text-gray-800 mb-6">My Transport Bookings</h1>
                    ...
                    {bookings?.length > 0 ? bookings.map(booking => (
                        <div key={booking._id} className="flex gap-4 bg-white rounded-2xl shadow-lg p-4 transform hover:scale-105 transition duration-300">
                            ...
                            <div className="py-3 pr-3 flex flex-col justify-between flex-grow">
                                <div>
                                    <h2 className="text-2xl font-semibold text-gray-800">{booking.transport ? booking.transport.title : 'No Title Available'}</h2>
