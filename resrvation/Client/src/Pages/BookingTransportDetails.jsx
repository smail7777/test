...
const BookingTransportDetailsPage = () => {
  const { id } = useParams();
  ...
  const fetchBookingDetails = async () => {
    try {
      const response = await axios.get(`/transport/bookings/${id}`);
      setBooking(response.data);
    } catch (error) {
      console.error('Error fetching transport booking details:', error);
    }
  };
  ...
  return (
    <div className="container mx-auto px-4 py-12">
      <div className="flex items-center mb-12">
        <h1 className="text-5xl font-extrabold text-center text-gray-900 w-full">Transport Booking Details</h1>
      </div>
      <div className="flex justify-start">
        <Link to="/account/my-transport-bookings" className="flex items-center text-gray-700 hover:text-gray-900">
      ...
      <div className="bg-white rounded-lg shadow-2xl p-10 transition duration-500 hover:shadow-xl">
        <h2 className="text-3xl font-bold mb-8 text-gray-800">{booking.transport.title}</h2>
...
export default BookingTransportDetailsPage;
