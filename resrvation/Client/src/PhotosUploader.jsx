...
export default function PhotosUploader({addedPhotos , onChange }){
    ...
    async function addPhotoByLink(ev) {
        ...
        try {
            ...
            onChange([...addedPhotos, filename]); // Ajouter le nouveau fichier à la liste existante
            setPhotoLink('');
        } catch (error) {
   ...
   <>
                        ...
                        <div className="mt-2 grid gap-2 grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
                            {addedPhotos.length > 0 && addedPhotos.map((filename, index) => (
                                <div key={index} className="h-32 flex relative">
                                    <img className="rounded-2xl w-full object-cover " src={`http://localhost:4000/uploads/${filename}`} alt={`Photo ${index}`} />
